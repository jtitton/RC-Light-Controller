# RC Car Smart Lighting System v7.0 — User Manual
...
| **[LuzesArduino_Producao.ino](LuzesArduino_Producao/LuzesArduino_Producao.ino)** | **Production & Track Racing** | **Zero Serial** (UART fully disabled, 100% standalone), I2C Fast-Mode (400kHz), loop speed **> 60,000 Hz**, 3D auto-vector alignment, roll-over hazard alert, and visual LED feedback. | **Flash: 11.5 kB (37%)**<br>**RAM: 363 B (17%)** |
...
| **[wokwi_diagram.json](wokwi_diagram.json)** | **[wokwi_diagram.json](wokwi_diagram.json)** | Complete wiring and visual component schematic for Wokwi online simulation (including MPU-6050). |

---

### ⚙️ Main System Features (v7.0)
...
- **CH6 Integrated Power Supply:** Powered directly through receiver channel CH6 via the ESC's Battery Elimination Circuit (BEC).
- **Smooth Fade Transitions:** Tail lights feature smooth ~300ms fade transitions.
- **Standalone Field Gesture Calibration:** Recalibrate all stick and headlight endpoints directly at the track by holding full steering deflection during power-up.
...
| :---: | :--- | :--- |
| **`W`** or **`w`** | **Forward Throttle** (+100%) | Brake OFF, $+0.40G$ longitudinal force. **+500 ms** per keypress. |
| **`S`** or **`s`** | **Brake / Reverse** (-100%) | Brake Lights and Tail Lights ON, $-0.60G$ deceleration force. **+500 ms** per keypress. |
| **`A`** or **`a`** | **Turn Left** (-100%) | Left Blinkers flash (120 bpm). **+500 ms** per keypress. |
| **`D`** or **`d`** | **Turn Right** (+100%) | Right Blinkers flash (120 bpm). **+500 ms** per keypress. |
...
| **`K`** or **`k`** | **Simulate Rollover Crash** | Toggles rollover hazard state (4-way flashers at 120ms). |
| **`I`** or **`i`** | **MPU-6050 Inertial Telemetry** | Displays $\vec{g}_0$, $\vec{u}_{\text{long}}$, and live longitudinal G-force. |
...
| **`Z`** or **`z`** | **Re-center Sticks (Trim)** | Measures stick neutral center positions and static gravity $\vec{g}_0$. |
